...
import { useAuthRedirect } from "@/src/hooks/useAuthRedirect"; // 🔑 Importamos el hook

export default function HomePage() {
  const loadingAuth = useAuthRedirect(); // 🔒 Bloquea si no hay sesión
  ...
  // 🔒 Si todavía estamos verificando sesión, mostramos loader
  if (loadingAuth) {
    return (
  ...
  return (
    ...
    >
      {" "}
      {loading && (
        <div className="text-white text-center text-xl mt-20">Cargando...</div>
      )}{" "}
      {error && (
        ...
        <>
          ...
          {searchQuery &&
            (filteredSongs.length > 0 || filteredArtists.length > 0) && (
              <div className="mb-12">
                {" "}
                <h2 className="text-4xl font-bold mb-8 mt-8 text-white">
          ...
          {(!searchQuery ||
            ...
            <div>
              {" "}
              <center>
                <h2 className="text-4xl font-bold mb-8 mt-16 text-white">
                  Artistas
                </h2>
              </center>{" "}
              <Carousel<Artist>
